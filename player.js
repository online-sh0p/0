...
let shuffleOrder = []; // shuffled index list
...
function getNextIdx() {
  ...
  if (isShuffled) {
    ...
    if (next === 0 && repeatMode === 0) return -1; // end of shuffle, no repeat
    return shuffleOrder[next];
  }
  const next = currentIdx + 1;
  if (next >= songs.length) return repeatMode === 1 ? 0 : -1;
  return next;
}
...
function getPrevIdx() {
  ...
  return currentIdx <= 0 ? songs.length - 1 : currentIdx - 1;
}

// ── NEXT / PREV / STOP ────────────────────────────────────
function nextSong() {
  const idx = getNextIdx();
  if (idx === -1) { audio.pause(); audio.currentTime = 0; document.body.classList.remove('playing'); return; }
  loadSong(idx);
}
function prevSong() {
...
btnShuffle.addEventListener('click', () => {
  ...
  btnShuffle.classList.toggle('mode-active', isShuffled);
...
btnRepeat.addEventListener('click', () => {
  ...
  if (repeatMode === 1) { btnRepeat.classList.add('mode-active');          showToast('Repeat ALL'); }
  if (repeatMode === 2) { btnRepeat.classList.add('mode-active','mode-one'); showToast('Repeat ONE'); }
  if (repeatMode === 0) { showToast('Repeat OFF'); }
  // Show "1" badge inside repeat button when repeat-one
  btnRepeat.querySelector('.repeat-one-badge').style.display = repeatMode === 2 ? 'block' : 'none';
});
